...
// 通过init初始化畸变模型获取remap(重映射)所需要的两个匹配参数map1 和map2 
// 通过这个函数对目标图像进行矫正
// 数据处理的流程和 opencv 里面的函数差不多
// 考虑到很多图像是未标定的，那么可以通过人为预估的方式制定相机畸变模型
// 具体看相机畸变原理
...
// 图像矫正
// 函数功能：计算无畸变和修正转换映射
// 这个函数主要用于计算无畸变和修正转换关系，为了重映射，将结果以映射的形式表达。
// 通过相机内参和畸变系数计算的和opencv 里面差不多
// 无畸变的图像看起来就像原始的图像，就好比这个图像是用内参为newCameraMatrix且无畸变的相机采集得到的。
// 在单目相机的情况下，newCameraMatrix通常等于cameraMatrix，
// 或者可以通过cv::getOptimalNewCameraMatrix来计算，以便更好地控制缩放
// https://mp.weixin.qq.com/s?__biz=MzU1MjY4MTA1MQ==&mid=2247484383&idx=1&sn=256dc7cfbeba4c907447e61be5acd3d1&chksm=fbff28ebcc88a1fd53efb60072e2048260fe435cc5588c4cce680a4939bda3b3404df2e60937&mpshare=1&scene=21&srcid=&pass_ticket=F2Q1WUTR54ocDmzv%20dLNZs4AjEZmzEcSvGv1rq4Ul/k8yXDw5NcV3HvBCJsUet6L#wechat_redirect
// https://zhuanlan.zhihu.com/p/69325943 opencv 里面的bug
// TODO 这里要改成手动标定计算模式
